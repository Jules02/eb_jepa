"""
Visualize the latent distance landscape d(s_0, s) = || E(s_0) - E(s) ||_2 over a
2D navigation environment (Two-Rooms or Maze), for a trained AC-Video-JEPA encoder.

We fix the layout (wall/door for two_rooms, a seeded maze for maze), pick an initial
ball position s_0, sweep the ball over every pixel position s, encode each rendered
frame with the trained encoder, and plot the L2 distance between each embedding and
the embedding of s_0 as a heatmap.

The env is read from the run's config.yaml (cfg.data.env_name), so the SAME script
works for both `two_rooms` and `maze` checkpoints.

Usage (the forward pass is tiny; CPU is fine):

    python -m examples.ac_video_jepa.viz_distance_landscape \
        --model_folder /lustre/work/.../impala_..._seed1 --s0 18,32

Args:
    model_folder : run directory containing `config.yaml` + a checkpoint.
    ...
    fix_wall     : two_rooms only — use the fixed wall/door layout (default True).
    seed         : maze only — seed the maze layout for reproducibility (default 0).
...
from eb_jepa.datasets.utils import _resolve_env, create_env, load_env_data_config
...
def _encode(encoder, obs_4d, normalizer, device, batch=1024):
    ...
    obs = normalizer.normalize_state(obs)          # env-specific preprocessing
...
def run(
    ...
    env_name = cfg.data.get("env_name", "two_rooms")

    # --- build the rendering env + matching normalizer (works for two_rooms and maze) ---
    _, ConfigClass, NormalizerCls = _resolve_env(env_name)
    ...
    if env_name == "two_rooms":
        data_overrides["fix_wall"] = fix_wall       # fixed wall makes the landscape readable
    data_overrides["device"] = str(device)
    env_config = update_config_from_yaml(
        ConfigClass, load_env_data_config(env_name, data_overrides)
    )
    env = create_env(env_name, env_config)
    env.rng = np.random.default_rng(seed)           # reproducible layout (esp. maze)
    # reset() (no location) sets up the layout: env.wall_img + img geometry. We ignore the
    # randomly-sampled start/target; coord_to_pixel reuses env.wall_img for the grid.
    env.reset()
    img_size = env.img_size

    # s_0 (pixel coords x,y). Default = center of the image. fire may parse "18,32" -> tuple.
    if s0 is None:
        s0_xy = [img_size / 2.0, img_size / 2.0]
    elif isinstance(s0, (tuple, list)):
        s0_xy = [float(v) for v in s0]
    else:
        s0_xy = [float(v) for v in str(s0).strip("()[]").split(",")]
    s0_t = torch.tensor(s0_xy, device=device, dtype=torch.float32)
    print(f"[viz] env={env_name} device={device} img_size={img_size} s0={s0_xy}")

    # --- encoder + weights ---
    encoder = _build_encoder(cfg, img_size, device)
    epoch = _load_encoder_weights(encoder, folder / checkpoint, device)

    # --- grid of ball positions s, rendered with the common coord_to_pixel API ---
    ...
    obs_grid = env.coord_to_pixel(locs.unsqueeze(1))[:, 0]       # (N, 2, H, W)
    obs0 = env.coord_to_pixel(s0_t.view(1, 1, 2))[:, 0]          # (1, 2, H, W)
    ...
    im = ax.imshow(
        dist, origin="upper", extent=[0, img_size, img_size, 0], cmap="viridis"
    )
    wall_np = env.wall_img.cpu().numpy()
    ax.imshow(
        np.ma.masked_where(wall_np == 0, wall_np),
        origin="upper", extent=[0, img_size, img_size, 0], cmap="gray_r", alpha=0.4,
    )
