...
class _FrameDecoder(nn.Module):
    ...
    The latent is full-resolution (h=w=128), so no upsampling is needed — this is a
    deeper *residual* conv stack mapping the D-channel latent back to the 2 chemical
    fields. Depth + normalisation matter here: Gray-Scott fields are nearly constant
    over large regions, so the variance-normalised VRMSE harshly punishes any failure
    to reproduce the fine spatial structure. A shallow net only learns the mean level
    (the old 3-conv decoder did exactly that -> floor VRMSE ~150). Each residual block
    is two 3x3 convs, so ``nblocks`` blocks give a receptive field of ~(4*nblocks+1) px.
    """
    ...
    if ckpt_path is not None:
        ...
        if "decoder" in ckpt:
            try:
                ...
                # cached decoder has a different architecture (e.g. trained before an
                # arch change) -> drop it and retrain from scratch
                print(f"[decoder] cached weights incompatible ({type(e).__name__}), "
                      f"retraining from scratch", flush=True)
        # No (usable) decoder weights — train from the checkpoint's frozen JEPA
